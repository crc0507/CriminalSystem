...
//													System.out.println("time:"+(timePart+1)+"----"+curNode.getId()+"--------tag"+tagNum+"嫌疑:"+closeRate2);
													...
													//将嫌疑指数存储到新的数据库中
													try (Transaction curTransaction = allGraphDBsArrayList.get(timePart).beginTx()){
														Long curNodeID = curNode.getId();
//														Node newCurNode = allGraphDBsArrayList.get(timePart).getNodeById(curNodeID);
														
														ResourceIterable<Node> newIterable = allGraphDBsArrayList.get(timePart).getAllNodes();
														ResourceIterator<Node> newIterator = newIterable.iterator();
														boolean isExist = false;
														while(newIterator.hasNext()) {
															Node temNode = newIterator.next();
															if(temNode.getProperty("id").equals(curNodeID)) {
																isExist=true;
															}
														}
														
														if(isExist==false) {
															Node newnewnNode = allGraphDBsArrayList.get(timePart).createNode(MyLabels.USERS);
															newnewnNode.setProperty("id", curNodeID);
															if(curNode.getProperty("name")!=null) {
																newnewnNode.setProperty("name", curNode.getProperty("name"));
															}
															String crimeString = "tag"+tagNum+"_crimerate";
															newnewnNode.setProperty(crimeString,closeRate2);
														}
														if(isExist==true) {
															System.out.println(tagNum);
															Node tempNode = allGraphDBsArrayList.get(timePart).getNodeById(curNodeID);
															String crimeString = "tag"+tagNum+"_crimerate";
															tempNode.setProperty(crimeString, closeRate2);
														}
														curTransaction.success();
														curTransaction.close();
													} catch (Exception e) {
														// TODO: handle exception
													}
//													GraphDatabaseService curGraphDatabaseService = allGraphDBsArrayList.get(timePart);
//													Transaction curTransaction = curGraphDatabaseService.beginTx();
//													Long curNodeID = curNode.getId();
//													Node newCurNode = curGraphDatabaseService.getNodeById(curNodeID);
//													newCurNode.setProperty("crimeRate",closeRate2);
//													curTransaction.success();
//													curTransaction.close();
													
//													if(closeRate2>=1.570361700832428) {
//														System.out.println("time:"+(timePart+1)+"----"+curNode.getId()+"--------tag"+tagNum+"嫌疑:"+closeRate2+"         犯罪");
//													}
//													if(closeRate2<1.570361700832428) {
//														System.out.println("time:"+(timePart+1)+"----"+curNode.getId()+"--------tag"+tagNum+"嫌疑:"+closeRate2+"         不犯罪");
//													}
